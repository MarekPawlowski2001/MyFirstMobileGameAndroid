...
public class GooglePlayStoreHelper implements StoreHelper, PurchasesUpdatedListener
{
	...
	private boolean bIsIapSetup = true;
				...
				public synchronized void onProductDetailsResponse(@NonNull BillingResult Result, @NonNull List<ProductDetails> ProductDetailsList)
				{
					...
					if(ResponseCode == BillingClient.BillingResponseCode.OK)
					{
						...
						if (AwaitingResponseCount == 0)
						{
							...
							Client.queryPurchasesAsync( QueryPurchasesParams.newBuilder().setProductType(BillingClient.ProductType.INAPP).build(), new PurchasesResponseListener() {
      								public void onQueryPurchasesResponse(BillingResult billingResult, List purchases) {
										Log.debug("onPurchaseRespones - Started");
										  if(billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK){
											Log.debug("purchasesHistoryList.size(): " + purchases.size());
											if(purchases.size()>0)
												DetailArrays.ProductIds[0] += " Owned";
										  }
										}
									}
								);
	...
	{
		Log.debug("[GooglePlayStoreHelper] - Beginning AcknowledgePurchase: " + PurchaseToken + "   - moja wersja");
	...
	{
		Log.debug("[GooglePlayStoreHelper] - Beginning ConsumePurchase: " + PurchaseToken + "   - moja wersja");
