...
void UInAppPurchaseReceiptsCallbackProxy::TriggerGetKnownReceipts(APlayerController* PlayerController)
{
	...
	//if (
	FUniqueNetIdRepl PurchasingPlayer = PlayerController->GetLocalPlayer()->GetUniqueNetIdForPlatformUser();// PurchasingPlayer.IsValid())
	{
		OnQueryReceiptsComplete(FOnlineError(true), PurchasingPlayer->AsShared());
	}
	//else
	{
		FFrame::KismetExecutionMessage(TEXT("UInAppPurchaseCallbackProxy::Trigger - Invalid player UniqueNetId Moja wersja 2"), ELogVerbosity::Warning);
	}
}
...
void UInAppPurchaseReceiptsCallbackProxy::TriggerGetOwnedProducts(APlayerController* PlayerController, bool bRestorePurchases)
{
	...
	//else
	{
		FFrame::KismetExecutionMessage(TEXT("UInAppPurchaseCallbackProxy::Trigger - Invalid player UniqueNetId Moja wersja 3"), ELogVerbosity::Warning);
	}
}
